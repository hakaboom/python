...
def TOUCH_EVENT():
    ...
    class _Touch:
        def __getitem__(self, item):
            if item == 'index':
                return list['INDEX_LIST']['index']
            if item == 'count':
                return list['INDEX_LIST']['count']
            if item == 'EVENT_ID' or item == 'EVENT_PATH':
                return list[item]
            if item == 'width' or item == 'height':
                return list['EVENT_WINDOWSIZE'][item]
...
def SCREEN_INFO():
    '''
        ...
        orientation 2 --home键在左
        orientation 3 --home键在下
        长边在width,短边在height
        坐标转换时有一种情况是event文件中的分辨率不等于机器的分辨率,这种时候要进行转换
    ...
    def system(self):
        ...
        class _System:
            ...
            def getForegroundApp(self):
                return obj.app_current()['package']

            def killApp(self, appPackage):
                obj.app_stop(appPackage)
            ...
            def init(self, index):
                ...
                SCREEN_INFO.setScreenSize(*obj.window_size())
                SCREEN_INFO.init(index)
        ...
        class _touch:
            def down(self, x, y, index=1):
                '''
                   ...
                   ABS_MT_SLOT(47) --手指index
                   ABS_MT_TRACKING_ID(57) -- 触摸的ID
                   ABS_MT_POSITION_X(53)  --触摸的x坐标
                   ABS_MT_POSITION_y(54)  --触摸的y坐标
                   ABS_MT_TOUCH_MAJOR(48) --指尖的宽度，以像素为单位
                   ABS_MT_PRESSURE(58) --触摸的压力
                ...
                l = (
                    ...
                    'sendevent {} {} {} {}'.format(eventPath, 3, 54, y * 10),
            ...
            def up(self, x, y, index=1):
                '''
                    sendevent /dev/input/event4 3 57 65535
